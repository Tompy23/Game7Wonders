...
public class CardFunctorPointsForNeighborsColorImpl implements CardFunctor 
{
	...
	{
		String out = "";
		out += "Add " + points + " point(s) for each ";
	...
	public void functor(Player7Wonders player, Card7Wonders card) 
	{
		for ( ColorType ct : colors )
		{
			player.getScore().addPoints( Util7Wonders.countOfCards( player.getLeft(), ct ) * points, card.getColor() );
			player.getScore().addPoints( Util7Wonders.countOfCards( player.getRight(), ct ) * points, card.getColor() );
			if ( self )
			{
